...
const Carousel = ({
  ...
  const syncScrollProgress = useCallback(
    () => ((scrollPosIndexRef.current + 1) / _itemsCount) * 100,
    [_itemsCount]
  );
  ...
  const [springs, set] = useSprings(_itemsCount, i => ({
    ...
    immediate: isStackAnimation
      ? i === (scrollPosIndexRef.current - 1 + _itemsCount) % _itemsCount
      : true
  }));
  ...
  useEffect(() => {
    ...
    // scroll animation
    scrollPosIndexRef.current = changeScrollPosIndex(
      scrollPosIndexRef.current,
      direction
    );
    ...
    set(i => {
      ...
      return {
        ...
        o,
        immediate: n =>
          (x <= -1 && direction < 0) || (x >= visible && direction > 0)
      };
    });
  }, [
    changeScrollPosIndex,
    getAnimatedValues,
    direction,
    isStackAnimation,
    selected,
  ...
  const onDrag = ({
    ...
    const _isSelectedStackItem =
      scrollPosIndexRef.current === index && isStackAnimation;
    const _onStart = scrollPosIndexRef.current === 0 && xDir > 0;
    const _onEnd =
      scrollPosIndexRef.current === _itemsCount - visible && xDir < 0;
    if (tap) {
    ...
    set(i => {
      ...
      if (isStackAnimation) {
        ...
        if (down && index === i) {
          peekRef.current =
            scrollPosIndexRef.current === i ? dragPeek : PEEK_THRESHOLD;
        }
        if (Math.abs(swipeX) && index === i && scrollPosIndexRef.current === i)
          peekRef.current = dragPeek;
      } else if (
        down &&
        ((!isInfinite &&
          ((_onStart && i < visible) ||
            (_onEnd && i >= _itemsCount - visible))) ||
          i === index)
      )
        // slideAnimation
  ...
  const onWheel = ({ direction: [xDir], velocity, elapsedTime, first }) => {
    ...
    if (isNewWheelRef.current) {
      ...
      const swipeByOne =
        ...
        elapsedTime <= 50;
      const swipeByTwo =
        velocity >= WHEEL_VELOCITY_THRESHOLD && elapsedTime <= 50;
  ...
  const bindWheel = useWheel(onWheel, {
    enabled: !isStackAnimation,
    axis: "x"
  });
      ...
      <div className="carousel" sx={styles.carousel} {...bindWheel()}>
        {springs.map((animatedValues, i) => (
          <a.div
            ...
            key={i}
            {...bindDrag(i)}
            style={_sxSprings(animatedValues)}
          >
            <a.div
