...
// Stack Effect
const stackAnimationSlideStyle = ({
  animatedValues,
  itemWidth,
  itemOffset = 0
}) => {
...
// Shift Effect
const shiftAnimationSlideStyle = ({
  animatedValues,
  visible,
  itemOffset = 0
}) => {
...
const shiftAnimationItemStyle = ({ animatedValues, visible }) => {
  ...
  const boxShadow = shadow.interpolate(
    _s => `0 ${_s}px ${2 * _s}px 0 rgba(0, 0, 0, 0.1)`
  );
...
const slideAnimationSlideStyle = ({ animatedValues, itemWidth, visible }) => {
  ...
  const display = x.interpolate(x_ =>
    x_ < range[0] || x_ > range[range.length - 1] ? "none" : "block"
  );

  const _x = x.interpolate({
    range,
    output: range.map(
      (idx, i) => idx * (100 + (1 - scOutput[i]) * (100 / scOutput[i]))
    ), // + (1 - scOutput[i]) * (idx < 0 ? -100 : 100)),
    extrapolate: "clamp"
  });

  const transform = interpolate(
    [_x, sc],
    (xt, s) => `scale(${s}) translateX(${xt}%)`
  );
